...
await page.waitForFunction(() => !document.querySelector('input[aria-label="Shorthand for this exercise"]'), { timeout: 15000 })
await page.evaluate(() => [...document.querySelectorAll('button')].find((b) => /Lineup$/.test(b.textContent.trim())).click())
await page.waitForFunction(() => (document.querySelector('[role=dialog]')?.textContent ?? '').includes('20 kg assist × 8·8'), { timeout: 15000 })
await sleep(400)

...
- **Rest timer is visual only** (v1.4, 2026-09-10): the line under the header fills as the rest elapses (`useRestTimer().progress`); at zero the pill turns green `go` and pulses. The v1.2 beep/vibrate ping was removed: creating an AudioContext (and setting `navigator.audioSession.type = 'playback'`) made the PWA take over the iPhone's audio session, which paused Spotify on every app switch, and the beep itself was unreliable in a PWA. A sound/haptic ping returns only with a native iOS/Android app.
- **Settings → Rest timer** (`gym_config.rest_override_seconds`; migration 0002, `rest_ping` dropped again in 0003): "Rest between sets" = Program (per-exercise, the default) or one duration for every exercise (60/90/120/150/180 s chips or a custom 15–600 s). Not seed-managed (`edited_at` untouched). `SessionView.rest.overrideSeconds`; `timer.start(rest.overrideSeconds ?? slot.restSeconds)`.
- **v1.3 polish after the first real workout (2026-09-08)**: (a) only the current set is an input row — logged sets stack above as compact rows (tap → change or `remove set`, which is the undo; no toast on log, the row flashes green for 1.2 s); (b) chips open an **in-app keypad** (`KeypadSheet`: display, ± by the exercise's stepping rule or ±1 for reps, a `last …` preset, digits, one `Set` button, "Wrong unit? Exercise settings") — never the native keyboard, which pushed the sheet half off-screen and left a blank band on iPhone; ✓ with no weight opens the keypad and logs on confirm; (c) rarely used actions (type it instead, swap, note, exercise settings) sit behind `•••` (`MoreSheet`); `postpone` stays visible; (d) every button has a pressed state (scale .97 + brightness, 80 ms) and `touch-action: manipulation`; bottom sheets get a drag handle and no X; (e) copy: header `Wrap up`, `Wrap up early`, check-in `Nice work.` / `Short and sweet.` with `templateName · min · sets` and one `I'm done` button; (f) toasts are a compact pill (2.5 s) for errors and rare confirmations only; (g) `KeyboardGapFix` snaps the scroll position back when the software keyboard closes (remaining text fields).

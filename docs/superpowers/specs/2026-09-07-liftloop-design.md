...
- **Vercel:** Git integration, Hobby plan. Every push to `main` is a production deploy. Env vars set in the Vercel dashboard: `APP_PASSCODE`, `SESSION_SECRET` (`openssl rand -base64 32`), `DATABASE_URL` (from the Neon integration), `TZ=Asia/Kolkata`, optional `COACH_EXPORT_TOKEN`.
